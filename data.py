...
class FERDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.train_data = self._get_dataset('train', self.transform_train)
            self.val_data = self._get_dataset('public', self.transform_test)

        if stage == 'test' or stage is None:
            self.test_data = self._get_dataset('private', self.transform_test)

    def _get_dataset(self, split, transform):
        # load data
        self.prepare_data() 
